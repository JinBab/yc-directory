  ...
  return (
    <form action={formAction} className='startup-form'>
        ...
        <div>
            ...
            {errors.link && <p className='startup-form__error'>{errors.link}</p>}
        </div>
        <div data-color-mode='light'>
            ...
            {errors.pitch && <p className='startup-form__error'>{errors.pitch}</p>}
        </div>

        <button type='submit' className='startup-form_btn text-white flex items-center justify-center' disabled={isPending}>
            {isPending? 'Submitting...' : ("Submit your Startup")}<Send className="size-6 ml-2"/>
        </button>
    </form>
  )
}
